...
- Modeled claims adjuster productivity using zero-inflated negative binomial GLM using package `pscl` as well as non-linear mixed modeling with package `brms`, developing theory for an alternative staffing criterion.
...
- Broad experience in several SQL frameworks (TSQL, DB2, Snowflake, `duckdb`) creating complex queries joining several tables each with hundreds of millions of records.
</div>
...
- These models used MCMC to make inferences about animal population size and relationships with spatial habitat covariates.
- The statistical models' performance was tested via simulation analysis using high-performance computing techniques using an original dual-layer parallelization scheme of a cluster of computers.
